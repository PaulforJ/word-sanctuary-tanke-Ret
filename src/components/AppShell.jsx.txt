      ...
      <aside className="md:w-60 shrink-0 border-b md:border-b-0 md:border-r border-[var(--color-line)] bg-[var(--color-surface)] flex md:flex-col justify-between">
        <div className="p-5 md:p-6">
          <div className="flex items-center gap-2.5 mb-1">
            <img src="/church-logo.jpg" alt="" className="h-8 w-8 object-contain mix-blend-multiply" />
            <h1 className="font-display text-base leading-tight text-[var(--color-primary)]">
              Word Sanctuary Tanke<br />Retention
            </h1>
          </div>
          <nav className="mt-7 hidden md:flex flex-col gap-1">
            {links.map((l) => (
              <NavLink
                key={l.to}
                to={l.to}
